...
namespace STGService.UI
{
    public class FWidget : FComponent
    {
        ...
        {
            EventHelper.AddListener(eventId, listener);
        ...
        private void _InitScheduler()
        {
            if (_interval >= 0f)
            {
                __scheduler = TimerHelper.Schedule(OnTick, _interval);
            }
        ...
        private void _RemoveEvent()
        {
            if (__listener != null)
            {
                ...
                {
                    EventHelper.RemoveListener(pair.Key, pair.Value);
                }
            }
        }
        ...
        private void _RemoveSchedule()
        {
            if (__scheduler != -1)
            {
                TimerHelper.Unschedule(__scheduler);
                __scheduler = -1;
            }
        }
