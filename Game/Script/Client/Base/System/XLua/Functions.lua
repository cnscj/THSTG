...
--便于derivedFunc扩展baseFunc，如baseFunc调用之前修改参数，或者baseFunc调用之后做其它处理.
...
--以服务器的时区格式化时间
os.localTime = os.time
function os.time(timeTb)
    if timeTb == nil then
        return os.localTime()
    else
        local time = os.localTime(timeTb)
        local date = os.localDate("*t", time)
        local dst = 0
